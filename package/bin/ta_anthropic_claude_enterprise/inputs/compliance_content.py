...
def _collect_content(
    ...
    if mode == "file_id":
        ...
        if not file_id:
            raise ValueError(
                "target_file_id is required when collection_mode is file_id"
            )
    ...
    if mode == "chat_id":
        ...
        if not chat_id:
            raise ValueError(
                "target_chat_id is required when collection_mode is chat_id"
            )
        chat_ids = [chat_id]
        ...
        if not user_email:
            raise ValueError(
                "target_user_email is required when collection_mode is user_email"
            )
